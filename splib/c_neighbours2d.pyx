""" Fast two dimensional lightweight neighbour list. """
...
def minimum_image_all(
    cnp.ndarray[DTYPE_t,ndim=2] dr,
    xmax,
    ymax):
    """ Applies the minimum image convention to the distance
        between two particles. 
    ...
    #cdef cnp.ndarray[cnp.float_t,ndim=2] 
    n = dr.shape[0]
...
def separations(
    ...
    npair = nl.shape[0]
    rij = np.zeros([npair,2],dtype=DTYPE)
...
def compress(
    ...
    for k in xrange(npair):
        ...
        drx = rij[k,0]
        dry = rij[k,1]
        rsquared = powf(drx,ex) + powf(dry,ex)
