...
!!
!!
!! Andrew Charles, RMIT 2006-2010.
!! With contributions from
!! Assoc Prof Peter Daivis, RMIT.
!!
!! Books on Smooth Particle Mechanics:
!!
!! 1. Smooth Particle Applied Mechanics - the state of the art. 
!!    William Hoover, World Scientific, 2005
!!
!! 2. Smoothed particle hydrodynamics: a meshfree particle method, 
!!  Liu G. R. and Liu M. B., World Scientific, 2003.
!!
!! Other authors of papers used to build this software:
!! JJ Monaghan, LG Sigalotti, D. Price.
!!
!! ================
!! Copyright Notice
!! ================
!! Copyright Andrew Charles and contributors
!! RMIT University 
!! This code is released under the Computer Physics Communications license. 
!! All other rights reserved.

! Main module: contains initialisation, main loop, global declarations
...
! bnl -- neighbour list for particle-boundary properties
! box -- simulation box
! tstat -- config data for the thermostat
! bounds -- config data for boundaries
...
!call rd_eos_input(input_file,1)
!call rd_kernel_input(input_file,1)
!call rd_boundary_input(bounds,input_file,1)
...
print *,nbounds,'boundary systems'

allocate(bp(nbounds))
allocate(bnl(nbounds))
...
! Initialise the boundary particles
do i=1,nbounds
  call destroy_particles(bp(i))
  ! assumption here that both boundaries (if 2) have the same number of parts
  call create_particles(bp(i),nbp_1,dimn)
  print *,'Created'
  call initialise_boundary_particles(bp(i))
  call rd_particle_input(bp(i),input_file,1)
  if(validate) call validate_particles(bp(i))
  print *,'Boundary particle data structure initialised ...'
end do

if(nbounds .eq. 1) then
  call set_lower_bounds(bp(1)%x)
endif

if(nbounds .eq. 2) then 
  call set_lower_bounds(bp(1)%x)
  call set_upper_bounds(bp(2)%x)
endif
...
if(nbounds .ge. 1) then 
  bp(1)%thermostat = boundary_thermostat_temperature(1)
endif

if(nbounds .eq. 2) then 
  bp(2)%thermostat = boundary_thermostat_temperature(2)
endif
...
do i=1,nbounds
  call rd_nlist_input(bnl(i),input_file,1)
  call init_nlist(bnl(i), cutoff, p%n, dimn, bp(i)%n)
  print *,'Boundary neighbour list initialised ...'
end do
...
if(nbounds .gt. 0) then
  call initialise(p,nl,box,bp,bnl)
else
...
!   reduce the timestep
  ...
  !write(*,'(F10.5)',advance='no') solver_sdt
  write(*,'(1A)',advance='no') '.'
