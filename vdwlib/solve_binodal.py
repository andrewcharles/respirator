...
V = np.arange(vmin,vmax,vmax/10000.,dtype="float64")
import sys
...
def calc_spinodal_points(v,t):
    ...
    for i in range(v.size):
        d[i] = diff(vdweos,v[i],t,0.0001)
    spline = interpolate.splrep(v,d,s=1)
...
def compute_binodes(v,T):
    nt =len(T) 
    binodes = np.zeros([nt,4])
    for i in range(nt):
        t = T[i]
        print 'finding binodes for T=',t
        p, bv1, bv2, bv3 = solve_binodes(v,t)
        binodes[i,0:4] = (t,p,bv1,bv3) 
    np.save('data/binodal_data.npy',binodes)
...
if __name__ == '__main__':
    ...
    if not exists('binodal_data.npy'):
    ...
    # Plot binodal points
    binodes = np.load('data/binodal_data.npy')
    nt = binodes.shape[0]
    for i in range(nt):
        pi = binodes[i,1]
        ti = binodes[i,0]
        #ax2.plot([binodes[i,2],binodes[i,3]],[ti,ti],'go')
        ax.plot([binodes[i,2],binodes[i,3]],[pi,pi],'go')
    ...
    plt.xlabel("Volume")
    plt.ylabel("Pressure")
    plt.title("van der Waals Equation, Water, T=%f Tscl" %T)
    ...
    plt.savefig("vdw_water.png",format='png')
